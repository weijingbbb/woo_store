...
class PopoverWidget extends StatelessWidget {
  ...
  void _show(BuildContext context) {
    ...
    Navigator.of(context).push(_PopupRoute(
      ...
      arrowColor: arrowColor ?? context.colors.scheme.surfaceContainer,
      showArrow: showArrow,
...
class _PopupRoute extends PopupRoute<void> {
  ...
  static final Rect _viewportRect = Rect.fromLTWH(
    _margin,
    _FirstScreen.statusBar + _margin,
    _FirstScreen.width - _margin * 2,
    _FirstScreen.height -
        _FirstScreen.statusBar -
        _FirstScreen.bottomBar -
        _margin * 2,
  );
  ...
  void _calculateChildOffset(Rect? childRect) {
    ...
    if (_maxHeight > bottomHeight) {
      // Above the target
      _bottom = _FirstScreen.height - targetRect.top;
